...
def main():
    ...
    wrist_circumference = st.number_input('Wrist Circumference (in mms)', min_value=0, max_value=2000, value=7)
    waist_circumference = st.number_input('Waist Circumference (in mms)', min_value=0, max_value=2000, value=32)
    neck_circumference = st.number_input('Neck Circumference (in mms)', min_value=0, max_value=2000, value=15)
    buttock_circumference = st.number_input('Buttock Circumference (in mms)', min_value=0, max_value=2000, value=40)
    thigh_circumference = st.number_input('Thigh Circumference (in mms)', min_value=0, max_value=2000, value=22)
    shoulder_length = st.number_input('Shoulder Length (in mms)', min_value=0, max_value=2000, value=18)
    chest_circumference = st.number_input('Chest Circumference (in mms)', min_value=0, max_value=2000, value=38)
    hand_length = st.number_input('Hand Length (in mms)', min_value=0, max_value=2000, value=7)
    chest_height = st.number_input('Chest Height (in mms)', min_value=0, max_value=2000, value=30)
    hip_breadth = st.number_input('Hip Breadth (in mms)', min_value=0, max_value=2000, value=15)
    ...
    if st.button('Predict Result'):
        ...
        (height > 75 ) or
        (weight > 240 ) or
        (wrist_circumference > 230 ) or
        (waist_circumference > 1000 ) or
        (neck_circumference > 420 ) or
        (buttock_circumference > 1100 ) or
        (thigh_circumference > 700 ) or
        (shoulder_length > 180 ) or
        (chest_circumference > 1200 ) or
        (hand_length > 210 ) or
        (chest_height > 1400 ) or
        (hip_breadth > 410 )   
